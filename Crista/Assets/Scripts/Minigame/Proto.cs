...
public class Proto : MonoBehaviour
{
    ...
    private int[,] goal_shape = new int[2,3] { {0, -14, -6 }, {0, -14, -7 } };

    private bool clicked = false;
    private int prev_gem_num = -1;
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0) && !clicked)  // if get mouse click
        {
            ...
            {
                int jewel_num = int.Parse(hit.collider.gameObject.name);
                ...
                bool all_same = true;
                for (int i = 1; i < goal_shape.GetLength(1); i++)
                {
                    if (jewel_num + goal_shape[row_n,i] < 0 || jewel_num + goal_shape[row_n,i] > 89)
                    {
                        all_same = false;
                        break;
                    }
                    if (jewels[jewel_num] != jewels[jewel_num + goal_shape[row_n,i]]){
                        all_same = false;
                        break;
                    }
                }
                ...
                // rotate jewel
                    ...
                    clicked = true;
                    StartCoroutine(turn_jewels(jewel_num));
                }
            }
        }
    }
    ...
    {
        int row_n = (n / 7) % 2;
        for (int i = 0; i < goal_shape.GetLength(1); i++)
        {
            jewels[n + goal_shape[row_n,i]] = -1;
            jewels_obj[n + goal_shape[row_n,i]].GetComponent<SpriteRenderer>().color = new Color(255, 255, 255);
        }
        ...
        for (int i = 0; i < goal_shape.GetLength(1); i++)
        {
            current_idx = top_idx = n + goal_shape[row_n, i];
